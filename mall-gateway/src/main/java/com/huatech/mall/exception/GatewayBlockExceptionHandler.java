...
public class GatewayBlockExceptionHandler extends DefaultBlockRequestHandler {
    ...
    private ResponseResult buildErrorResult(Throwable ex) {
        if(ex instanceof ParamFlowException) {
            return  ResponseResult.failure(ApiErrorCodeEnum.TOMANY_REQUEST_ERROR);
        }else if (ex instanceof DegradeException) {
            return ResponseResult.failure(ApiErrorCodeEnum.BACKGROUD_DEGRADE_ERROR);
        }else{
            return ResponseResult.failure(ApiErrorCodeEnum.BAD_GATEWAY);
        }
    }
